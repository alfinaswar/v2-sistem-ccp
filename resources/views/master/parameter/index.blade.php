...
@section('content')
    ...
    <div class="row mb-3">
        <div class="col text-end">
            <a class="btn btn-primary" href="{{ route('parameter.create') }}">Tambah Parameter Baru</a>
        </div>
    </div>
    ...
    <script>
        $(document).ready(function() {
            $('body').on('click', '.btn-delete', function() {
                ...
                Swal.fire({
                    title: 'Hapus Data?',
                    text: "Apakah Anda yakin ingin menghapus parameter ini?",
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ route('parameter.destroy', ':id') }}'.replace(':id',
                                id),
                            type: 'DELETE',
                            data: {
                                _token: '{{ csrf_token() }}'
