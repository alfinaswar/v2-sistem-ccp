    ...
    <script>
        $(document).ready(function() {
            $('body').on('click', '.btn-delete', function() {
                ...
                Swal.fire({
                    title: 'Hapus Data?',
                    text: "Apakah Anda yakin ingin menghapus jenis pengajuan ini?",
                    icon: 'warning',
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: '{{ route('jenis-pengajuan.destroy', ':id') }}'.replace(
                                ':id', id),
                            type: 'DELETE',
                            data: {
                                _token: '{{ csrf_token() }}'
                            ...
                            success: function(response) {
                                if (response.status === 200) {
                                    ...
                                    $('#jenisPengajuanTable').DataTable().ajax.reload();
                                } else {
            ...
            function loadDataTable() {
                $('#jenisPengajuanTable').DataTable({
                    responsive: true,
                    serverSide: true,
                    processing: true,
                    bDestroy: true,
                    ajax: {
                        url: "{{ route('jenis-pengajuan.index') }}",
                    },
                    language: {
